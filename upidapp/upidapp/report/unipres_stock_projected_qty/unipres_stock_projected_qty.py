...
def execute(filters=None):
	...
	show_shortage_only = filters.get("shortage_qty")
	columns = get_columns()
...
# ADD FILTER HERE: IF filter condition met: continue -------------------------------------------------------
		if show_shortage_only and shortage_qty <= 0:
			continue
					
		data.append([item.name, item.item_name, item.item_group, item.brand, bin.warehouse,
...
def get_bin_list(filters):
	...
	bin_list = frappe.db.sql("""select item_code, warehouse, actual_qty, indented_qty,
...
def get_item_map(item_code, include_uom):
	...
	items = frappe.db.sql("""
		select item.name, item.item_name, item.item_group, item.brand, item.stock_uom{cf_field}
		from `tabItem` item	
